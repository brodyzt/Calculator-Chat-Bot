...
val add: Eval.integer -> Eval.integer -> Eval.integer -> Eval.value

(*[subtract a b n] is the unique 0 <= r < n such that (a - b - r) divides n
  if n <= 0 then the result will be an excpetion value*)
val subtract: Eval.integer -> Eval.integer -> Eval.integer -> Eval.value

(*[multiply a b n] is the unique 0 <= r < n such that (a*b - r) divides n
Precondition: n > 0*)
val multiply: Eval.integer -> Eval.integer -> Eval.integer -> Eval.value

(*[divide a b n] is the unique 0 <= r < n such that (a*b' - r) divides n, where
  if [multiply b b' n] is 1, or -1 if no such b' exists
  if n <= 0 then the result will be an excpetion value*)
val divide: Eval.integer -> Eval.integer -> Eval.integer -> Eval.value
...
(*[power a b n] is the unique 0 <= r < n such that (a^b -r) divides n if b >=0,
  ...
  if n <= 0 then the result will be an excpetion value*)
val power: Eval.integer -> Eval.integer -> Eval.integer -> Eval.value
...
val gcd: Eval.integer -> Eval.integer -> Eval.value

(*[lcm a v] is the smallest natural number n
  such that a divides n and v divides n*)
val lcm: Eval.integer -> Eval.integer -> Eval.value
...
val gen_prime: Eval.integer -> Eval.value
...
val factor: Eval.integer -> Eval.value
...
(*[totient n] is the number of units in Z mod n. In other words it is the
  ...
  if n <= 0 then the result will be an excpetion value*)
val totient: Eval.integer -> Eval.value
